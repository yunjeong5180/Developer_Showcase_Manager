  ...
  async getDashboardStats() {
    try {
      ...
      if (!viewCountResult.error && viewCountResult.data) {
        totalViews = viewCountResult.data.reduce(
  ...
  async getActivitySummary(days = 30) {
    try {
      ...
      activities.forEach((activity) => {
        activityCount[activity.action_type] =
          (activityCount[activity.action_type] || 0) + 1;
      });
      ...
      const summary = {
        ...
        mostFrequentAction: Object.keys(activityCount).reduce(
...
// 개별 함수 export (편의를 위해)
export const getDashboardStatistics = statisticsAPI.getDashboardStats;
export const getRecentActivities = statisticsAPI.getRecentActivities;
export const getTopViewedProjects = statisticsAPI.getTopViewedProjects;
export const getActivityStats = statisticsAPI.getActivityStats;
