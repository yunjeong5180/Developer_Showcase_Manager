...
export const imageAPI = {
  ...
  async uploadProjectImages(files, projectId) {
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const fileName = `${user.id}/projects/${projectId}/${Date.now()}_${
          i + 1
        }.${fileExt}`;
      ...
      console.log(
        `프로젝트 이미지 업로드 완료: ${uploadedImages.length}개 파일`
      );
      return {
        success: true,
        error: null,
  ...
  async deleteProjectImages(imagePaths) {
    try {
      ...
      const userFilePaths = filePaths.filter((path) =>
  ...
  async uploadWithProgress(files, projectId, onProgress) {
    try {
      ...
      console.log(
        `배치 업로드 완료: ${successful.length}개 성공, ${failed.length}개 실패`
      );
