...
export default function UserPortfolio() {
  ...
  const fetchPortfolioData = async () => {
    try {
      ...
      // 프로필 정보 가져오기
      const profileRes = await fetch(`http://localhost:3000/api/profiles/public/${username}`)
      if (!profileRes.ok) {
      ...
      // 기술 스택 가져오기
      const skillsRes = await fetch(`http://localhost:3000/api/user-skills/user/${username}`)
      if (skillsRes.ok) {
      ...
      // 프로젝트 가져오기
      const projectsRes = await fetch(`http://localhost:3000/api/projects/user/${username}`)
      if (projectsRes.ok) {
